...
class DouBaoModelTester(BaseModelTester):
    """豆包模型测试器，继承自BaseModelTester"""
    ...
    def _setup_correct_paths(self):
        ...
        # 获取当前文件路径
        current_file_path = os.path.abspath(__file__)
        # 当前文件在: try/Agents/doubao_main.py
    ...
    def create_websearch_tool(self):
        ...
        try:
            ...
            # 检查doubao_websearch.py文件
            websearch_file = os.path.join(self.llm_tools_dir, "doubao_websearch.py")
            if not os.path.exists(websearch_file):
                raise FileNotFoundError(f"doubao_websearch.py文件不存在：{websearch_file}")

            # 使用动态导入确保路径正确
            spec = importlib.util.spec_from_file_location("doubao_websearch", websearch_file)
            doubao_websearch_module = importlib.util.module_from_spec(spec)

            # 确保模块可以找到其他依赖
            if self.llm_tools_dir not in sys.path:
                sys.path.insert(0, self.llm_tools_dir)

            spec.loader.exec_module(doubao_websearch_module)

            # 调用创建函数
            websearch_tool = doubao_websearch_module.create_doubao_websearch_tool()
